...
function s.initial_effect(c)
	...
	c:EnableReviveLimit()
	Fusion.AddProcMix(c,true,true,29292929,aux.FilterBoolFunctionEx(Card.IsSetCard,0x1122))
...
function s.contactfilter(c)
	return c:IsSetCard(0x1122) and c:IsAbleRemoveAsCost()
end
...
function s.atkfilter(c)
	return (c:IsSetCard(0x8) or c:IsSetCard(0x1122) or (c:IsRace(RACE_WARRIOR) and c:IsSetCard(0x1122))) and c:IsMonster()
end
...
function s.costfilter(c)
	return (c:IsSetCard(0x1122) or (c:IsSetCard(0x8) and c:IsRace(RACE_WARRIOR))) and c:IsMonster() and c:IsAbleToRemove()
end
